...
import com.arkeup.link_innov.gestion_profil_mcs.donnee.dto.RelationShipDTO;
...
public class ProfilRSAImpl implements ProfilRSA {
    ...
    public ProfilDTO getAuthInformation(String username) {
        ...
        if (profilDTO != null) {
            ...
            if (activeSubscriptionDTO != null) {
                SubscriptionDTO subscriptionDTO = abonnementMCS.getSubscription(activeSubscriptionDTO.getIdSubscription());
    ...
    private DeferredResult<ProfilDTO> generateExport(String token, String userId, Profil profil) {
        ...
        ForkJoinPool.commonPool().submit(() -> {
            ...
            try {
                ...
                mediaDTO = mediaMCS.uploadFile(token, mediaDTOToken.getId(), base64zip, MediaType.DOCUMENT.name().toLowerCase() + "/" + zipFileName);
    ...
    @Override
    public CustomPageDTO<ProfilDTO> getPaginatedProfil(List<String> userIds, String filter, String categorie, Pageable pageable) {
        CustomPageDTO<ProfilDTO> customPageDTO = new CustomPageDTO<>();
