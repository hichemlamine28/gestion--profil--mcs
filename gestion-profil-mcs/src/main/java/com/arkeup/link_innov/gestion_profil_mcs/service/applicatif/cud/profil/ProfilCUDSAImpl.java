...
public class ProfilCUDSAImpl implements ProfilCUDSA {
    ...
    public Profil initProfil(String uuid, String username, String email, String webSite, String lastName, String chatId) {
    ...
    public ProfilDTO setProfilHasMedia(ProfilHasMediaDTO profilHasMediaDTO) {
        ...
        if (profilHasMediaDTO.getType() != null) {
            if (profilHasMediaDTO.getType().equals("MEDIA"))
                ...
                profil.setHasBackground(profilHasMediaDTO.getHasMedia() != null ? profilHasMediaDTO.getHasMedia() : false);
    ...
    private boolean updateProfilHasMedia() {
        ...
        List<Profil> profilList = profilRSM.findAll()
                .stream()
        ...
        if (profilDTOS != null && profilDTOS.size() > 0) {
            ...
            // MEDIA
            List<MapMediaDTO> mapMediaDTOS = profilDTOS
                    .stream()
            ...
            if (mapMediaDTOS.size() > 0) {
                ...
                if (postIds.size() > 0) {
                    List<ProfilDTO> profilDTOList = profilDTOS
                            .stream()
                            .filter(profilDTO -> postIds.contains(profilDTO.getId()))
                            .map(this::mapProfilToHasMedia)
                            .collect(Collectors.toList());
            ...
            // BACKGROUND
            List<MapMediaDTO> mapMediaBackgroundDTOS = profilDTOS
                    .stream()
            ...
            if (mapMediaBackgroundDTOS.size() > 0) {
                ...
                if (postIds.size() > 0) {
                    List<ProfilDTO> profilDTOList = profilDTOS
                            .stream()
                            .filter(profilDTO -> postIds.contains(profilDTO.getId()))
                            .map(this::mapProfilToHasBackground)
                            .collect(Collectors.toList());
                    List<Profil> posts = profilCUDSM.updateAllProfil(profilFactory.toDOs(profilDTOList));
